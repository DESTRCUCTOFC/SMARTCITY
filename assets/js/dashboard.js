...
const renderProfile = (user, profile) =>{
    ...
    console.log('hola terminando funcion')
}
...
const reloadProfileAndWeather = async () => {
    ...
    console.log('hola despues de funcion') 
...
const loadWeather = async (city) =>{
    if(!city){
        hideWeatherContent()
        showWeatherAlert('No hay ciudad')
        return
    }
...
editProfileForm?.addEventListener('submit', async (event)=>{
    ...
    try {
        ...
        showAlert('profileAlert', error.message || 'No se puede actualizar')
    } finally{
        setButtonLoading(
            saveProfileBtn,
            false,
